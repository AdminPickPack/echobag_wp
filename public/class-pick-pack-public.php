...
class Pick_Pack_Public
{
	...
	public function plugin_not_setup()
	{
	 ...
	 * Add model
	 */
	//Make a function that checks eco bag is eligible,if its in the cart and not eligible;remove it from cart. If its not in the cart and eligible show the popup or add eco button
		...
		foreach ($order->get_items() as $item_id => $item) {
			...
			if ($item_data['name'] == "Pick Pack") {
				...
				if ($stock_quantity > 0) {
					...
					update_option('eco_bag_stock', $new_stock_quantity);
				}

				//eco_bag_token wont be present if split payment not there, cater for that
				$eco_bag_token = '';
				if ($split_payment) {
					$eco_bag_token = get_option('temp_eco_bag_token');
				} else {
					$eco_bag_token = get_option('eco_bag_token');
				}
	...
	public function curl_webhook_receive()
	{
		if (isset($_GET['request']) && $_GET['request'] === 'curl' && SERVER_URL . 'dashboard/update.php' === $_SERVER['HTTP_REFERER']) {
			...
			if ($_product !== null && $_product !== false && $split_payment) {
				update_option('eco_bag_price', $_GET['price']);
				$_product->set_regular_price($_GET['price']);
				$_product->save();
				$new_price = $_product->get_regular_price();
				echo json_encode(['message' => "success", "new_price" => $new_price]);
			} else {

				echo json_encode(['message' => "failure", "new_price" => '']);
			}
	...
	public function display_pick_pack_info()
	{
		if (!is_ajax()) {
			...
			$text = esc_attr__('Pick Pack bag option only available in Canada and depending on the products being bought', 'pick-pack');
	...
	public function return_from_company_details()
	{
		...
		if (isset($_GET['company_details'])) {
			...
			if (isset($_GET['token'])) {
				...
				update_option('temp_eco_bag_token', $_GET['token']);
				// code...
			}

			exit(wp_redirect(get_dashboard_url() . 'admin.php?page=pick-pack&company_details=true'));
		}
	}
	...
	public function checkout_popup()
	{
		...
		require_once plugin_dir_path(dirname(__FILE__)) . 'public/partials/checkout-popup.php';
